...
# A* Search, Dijkstra, Greedy BFS, DFS, dan Kruskal Maze Generator
# KidsCanCode 2017 - Modified with DFS and Kruskal
import pygame as pg
from os import path
import heapq
import random
...
def generate_recursive_backtrack_maze(width, height):
    ...
    Algoritma yang lebih stabil dan reliable untuk maze generation
    """
    ...
    # Pilih starting cell (biasanya random, tapi kita mulai dari dekat start)
...
while running:
    ...
    # Fill explored area with different colors for different algorithms
    explore_color = EXPLORED
    for node in path:
        x, y = node
        rect = pg.Rect(x * TILESIZE, y * TILESIZE, TILESIZE, TILESIZE)
        pg.draw.rect(screen, explore_color, rect)
        
    draw_grid()
    g.draw()
    
    # draw path from start to goal - FIXED: Skip first arrow to avoid overlap with start icon
    current = start
    l = 0
    ...
    draw_text('K: Generate Maze (Recursive Backtrack)', 20, NAVY_BLUE, 10, 35)
    draw_text('S: Generate Simple Maze', 20, NAVY_BLUE, 10, 60)
    draw_text('R: Reset Original Maze', 20, NAVY_BLUE, 10, 85)
    draw_text('C: Clear All Walls', 20, NAVY_BLUE, 10, 110)
    draw_text('Mouse: Left=Wall, Middle=Start, Right=Goal', 20, NAVY_BLUE, 10, 135)
